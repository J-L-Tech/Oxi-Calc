...
pub fn make_3d_curve_from_data(
    x_data: &Vec<f64>,
    y_data: &Vec<f64>,
    z_data: &Vec<f64>,
) -> Result<Curve, StrError> {
    let mut curve = Curve::new();
    curve.set_line_width(2.0);
...
pub fn make_graph(graph_info: GraphInfo) -> Result<(), StrError> {
    ...
    if graph_info.is_2d {
        ...
        plot.set_title(graph_info.title.as_str())
...
mod graph_maker_tests {
    ...
    fn make_simple_2d_curve() {
        ...
        assert!(make_2d_curve_from_data(&vec, &vec).is_ok());
    }
    ...
    fn make_simple_3d_curve() {
        ...
        assert!(make_3d_curve_from_data(&vec, &vec, &vec).is_ok());
    }
}
